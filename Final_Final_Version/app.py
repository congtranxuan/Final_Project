...
def registration():
    ...
    return render_template("members.html", contents=alist)
...
def face_compare():
    ...
    for i in range(len(df["fullname"])):
        ...
        source_face, matches = compare_faces(
            SOURCE_BUCKET, KEY_SOURCE, TARGET_BUCKET, KEY_TARGET)
        print(KEY_TARGET)
        data = {}
    ...
    imglink = "https://sourceimageforrekognition.s3.us-east-2.amazonaws.com/" + \
        'face_compare_image.jpg'
    upload_imglink = [imglink]
...
def upload_file(file_name, bucket):
    ...
    response = s3_client.upload_file(file_name, bucket, object_name, ExtraArgs={
                                     "ContentType": "image/jpg", 'ContentDisposition': 'inline', 'ACL': 'public-read'})
...
@app.route("/cvatlarge")
def cvatlarge():
    return render_template("cvatlarge.html")
